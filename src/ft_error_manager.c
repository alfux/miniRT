...
int	ft_error_manager(int err, int fd)
{
	...
	if (err > 99 && errno)
		...
		ft_putstr_fd("Invalid extension format.\n MiniRT works only with `.rt' extension files.\n", 2);
	else if (err == 2)
		ft_putstr_fd("Invalid object's definition.\nValid definition are <camera>`C'\t <light>`L'\t<ambiant light.`L'\t<sphere>`sp'\t<plan>`pl'\t<cylinder>`cy'\t<ellipse>`el'\t<hyperbol>`hy'\t<parabol>`pa'\t<cone>`co'\t<.obj filed extension>`ob'\n", 2);
	else if (err == 3)
		ft_putstr_fd("No camera found.\n The execution need one camera with format `C\t<position>x,y,z\t\t<orientation vector>x,y,z\t\t<vision ham.[0-180] to turn on.\n", 2);
	else if (err == 4)
