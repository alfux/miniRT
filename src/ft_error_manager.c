...
/*   Updated: 2023/02/23 17:14:27 by efunes           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_error_manager(int err, int fd)
{
	...
	if (!err && errno)
		...
		ft_putstr_fd("No camera found. The execution need one camera with format `C\t<position>x,y,z\t\t<orientation vector>x,y,z\t\t<vision ham.[0-180] to turn on.\n", 2);
	else if (err == 4)
