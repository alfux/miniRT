...
/*   Updated: 2023/02/28 02:59:02 by alfux            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vec	ft_pblbmp(t_vec const *vtx, t_vec const *nml, t_pbol const *pbl)
{
	...
	if (ft_det3x3(pbl->bns.bmp.bas) == 0.f || !pbl->bns.bmp.map.iid)
		return (*nml);
	basz = ft_setvec(pbl->bns.bmp.bas.top.z, pbl->bns.bmp.bas.mid.z, pbl->bns.bmp.bas.bot.z);
	...
	return (ft_multmv(ft_rotnml(&pbl->bns.bmp.bas, nml, -acos(ft_scalar(basz, *nml))),
			ft_multmv(pbl->bns.bmp.bas, ft_bmpmap(&pbl->bns.bmp.map,
			&pbl->bns.bmp, phi * cos(theta) / sqrt(pbl->rat.x), phi * sin(theta) / sqrt(pbl->rat.y)))));
}
